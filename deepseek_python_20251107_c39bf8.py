...
# Initialize system - INI HARUS DILAKUKAN SEBELUM ROUTES DIPANGGIL
auto_poster = None

def initialize_auto_poster():
    """Initialize the auto posting system"""
    global auto_poster
    try:
        ...
        auto_poster = type('FallbackAutoPoster', (), {
            'scheduled_posts': [],
            'bulk_titles': [],
            'posting_config': DEFAULT_CONFIG.copy(),
            'add_bulk_titles': lambda self, titles, keywords_map=None: len(titles),
            'save_data': lambda self: None,
            'process_scheduled_posts': lambda self: None
        })()
        auto_poster.scheduled_posts = []
        auto_poster.bulk_titles = []
        auto_poster.posting_config = DEFAULT_CONFIG.copy()

# Initialize the system
initialize_auto_poster()
...
# Routes - SEMUA ROUTES HARUS DITULIS SETELAH INISIALISASI auto_poster
@app.route('/')
def index():
    """Main dashboard"""
    try:
        ...
        # Ensure auto_poster is available
        if auto_poster is None:
            initialize_auto_poster()
        ...
        return "System Error - Please check the application logs", 500
...
def health_check():
    ...
    try:
        return jsonify({
            ...
            "posts_count": len(auto_poster.scheduled_posts) if auto_poster else 0,
            "bulk_titles_count": len(auto_poster.bulk_titles) if auto_poster else 0,
            "api_configured": api_keys_manager.keys.get('is_configured', False),
...
def debug_info():
    ...
    debug_info = {
        ...
        "scheduled_posts_count": len(auto_poster.scheduled_posts) if auto_poster else 0,
        "bulk_titles_count": len(auto_poster.bulk_titles) if auto_poster else 0,
        "api_configured": api_keys_manager.keys.get('is_configured', False),
        "auto_poster_initialized": auto_poster is not None
    }
...
def scheduler_status():
    """Endpoint untuk mengecek status scheduler"""
    try:
        status = {
            "scheduler_active": scheduler_thread.is_alive() if 'scheduler_thread' in globals() else False,
            "next_run": str(schedule.next_run()) if schedule.next_run() else "No scheduled jobs",
            "scheduled_posts_count": len([p for p in auto_poster.scheduled_posts if p.get('status') == 'scheduled']),
            "pending_bulk_titles": len([t for t in auto_poster.bulk_titles if t.get('status') == 'pending']),
...
def trigger_scheduler():
    ...
    try:
        ...
        return jsonify({"message": "Scheduler triggered successfully"})
    except Exception as e:
...
if __name__ == '__main__':
    ...
    logger.info(f"✅ AutoPostingSystem initialized: {auto_poster is not None}")
