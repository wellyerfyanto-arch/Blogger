...
def process_csv_file(file):
    ...
    try:
        ...
        if 'keywords' in headers:
            keyword_index = headers.index('keywords')
        
        for row in reader:
            if row and len(row) > title_index and row[title_index].strip():
                title = row[title_index].strip()
                titles.append(title)
                
                if keyword_index is not None and len(row) > keyword_index and row[keyword_index].strip():
                    keywords = [k.strip() for k in row[keyword_index].split(',')]
...
@require_auth
def bulk_upload():
    """Handle bulk upload of titles without pandas"""
    try:
