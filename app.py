...
class APIKeysManager:
    ...
    def update_keys(self, new_keys):
        ...
        try:
            ...
            # Fix: More flexible configuration check
            self.keys['is_configured'] = any([
                self.keys['openai_api_key'],
                self.keys['hf_api_key']
            ]) and bool(self.keys['blogger_blog_id'])
...
class AutoPostingSystem:
    ...
    def load_data(self):
        """Load data from files with initialization for missing files"""
        # Initialize files if they don't exist
        default_files = {
            'data/scheduled_posts.json': [],
            'data/posting_config.json': DEFAULT_CONFIG,
            'data/bulk_titles.json': []
        }
...
def login():
    ...
    try:
        if request.method == 'POST':
            ...
            # First time setup - no master key exists
            if not os.path.exists(api_keys_manager.master_key_file):
                api_keys_manager.set_master_key(master_key)
                session['authenticated'] = True
                session.permanent = True
                logger.info("Master key set for first time and user logged in")
                return redirect(url_for('index'))
            
            # Existing master key - verify
        ...
        # Show first-time setup message if no master key exists
        first_time = not os.path.exists(api_keys_manager.master_key_file)
...
def update_keys():
    ...
    try:
        new_keys = {
            ...
            'blogger_blog_id': request.form.get('blogger_blog_id', ''),
            'google_client_id': request.form.get('google_client_id', ''),
            'google_client_secret': request.form.get('google_client_secret', '')
        }
        
        if api_keys_manager.update_keys(new_keys):
            return jsonify({'success': True, 'message': 'API keys updated successfully'})
        else:
            return jsonify({'success': False, 'message': 'Failed to update API keys'}), 500
            
    except Exception as e:
        logger.error(f"Error updating API keys: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/upload_titles', methods=['POST'])
@require_auth
def upload_titles():
    """Upload titles from file"""
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            added_count = auto_poster.add_bulk_titles(titles, keywords_map)
            
            return jsonify({
                'success': True, 
                'message': f'Successfully added {added_count} titles from {filename}',
                'count': added_count
            })
        else:
            return jsonify({'success': False, 'message': 'Invalid file type. Only CSV and TXT files are allowed.'}), 400
            
    except Exception as e:
        logger.error(f"Error uploading titles: {str(e)}")
        return jsonify({'success': False, 'message': f'Upload failed: {str(e)}'}), 500

@app.route('/api/add_titles', methods=['POST'])
@require_auth
def add_titles():
    """Add titles manually"""
    try:
        ...
        added_count = auto_poster.add_bulk_titles(titles)
        
        return jsonify({
            'success': True,
            'message': f'Successfully added {added_count} titles',
            'count': added_count
        })
        
    except Exception as e:
        logger.error(f"Error adding titles: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/schedule_posts', methods=['POST'])
@require_auth
def schedule_posts():
    """Schedule posts from bulk titles"""
    try:
        ...
        titles_to_schedule = pending_titles[:count]
        
        if not titles_to_schedule:
            return jsonify({'success': False, 'message': 'No pending titles available'}), 400
        
        scheduled_count = 0
        current_date = datetime.strptime(start_date, '%Y-%m-%d').date()
        config = auto_poster.posting_config['posting_schedule']
        
        for title_data in titles_to_schedule:
            try:
                post_data = {
                    ...
                    'scheduled_at': datetime.now(TIMEZONE).isoformat()
                }
                ...
                logger.error(f"Error scheduling title {title_data.get('title')}: {str(e)}")
                continue
...
def update_config():
    ...
    try:
        new_config = auto_poster.posting_config.copy()
        
        # Update posting schedule
        if 'posting_schedule' in request.json:
            schedule_config = request.json['posting_schedule']
            if 'frequency' in schedule_config:
                new_config['posting_schedule']['frequency'] = schedule_config['frequency']
            if 'time' in schedule_config:
                new_config['posting_schedule']['time'] = schedule_config['time']
            if 'days' in schedule_config:
                new_config['posting_schedule']['days'] = schedule_config['days']
            if 'max_posts_per_day' in schedule_config:
                new_config['posting_schedule']['max_posts_per_day'] = schedule_config['max_posts_per_day']
        
        # Update content settings
        if 'content_settings' in request.json:
            content_config = request.json['content_settings']
            for key in content_config:
                if key in new_config['content_settings']:
                    new_config['content_settings'][key] = content_config[key]
        
        # Update SEO settings
        if 'seo_settings' in request.json:
            seo_config = request.json['seo_settings']
            for key in seo_config:
                if key in new_config['seo_settings']:
                    new_config['seo_settings'][key] = seo_config[key]
        
        auto_poster.posting_config = new_config
        auto_poster.save_data()
        auto_poster.setup_scheduler()
...
def generate_article_api():
    ...
    try:
        ...
        seo_analysis = analyze_seo(article_data['content'], title, keywords)
